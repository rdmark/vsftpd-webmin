...
sub instance() {
	return instance_file($config{"vsftpd_config_file"});
}
...
sub read_config_userdb() {
	...
	my $pam_file = read_file_contents($self->get_pam_file());
...
sub get_pam_file() {
	...
	return $config{'pam_service_dir'} . "/" . $self->config_instance('pam_service_name')->value();
}
...
sub persist_userdb($) {
	...
	if (!exists $config{'pam_service_dir'} ||
	...
	my $contents = read_file_contents($pam_filename);
	...
	my $template = read_file_contents(module_root_directory('vsftpd') . "/vsftpd_pam_d_template");
	$template = substitute_template($template, {'USERDB'=>$vsftpd_db_name});

	lock_file($pam_filename);
	open (FILE, ">$pam_filename");
	print FILE $template;
	close FILE;
	unlock_file($pam_filename);

	webmin_log("persist", "main_config", "userdb", undef);
}
...
sub persist {
	...
	if (!$self->can_rollback("_perm")) {
		rename_logged($self->{CONFIG_FILENAME}, $self->backup_name("_perm"));
	}
	else {
		rename_logged($self->{CONFIG_FILENAME}, $self->backup_name());
	}
	lock_file($self->{CONFIG_FILENAME});

	warn("writing: " . $self->{CONFIG_FILENAME});
	write_file($self->{CONFIG_FILENAME}, \%contents);
	unlock_file($self->{CONFIG_FILENAME});

	webmin_log("persist", "main_config", "main", undef);
...
sub rollback {
	...
	if ($self->can_rollback($type)) {
		warn("rollback: " . rename_logged($self->backup_name($type), $self->{CONFIG_FILENAME}));
		webmin_log("rollback", "main_config", $type ? $type : "oneversion", undef);
	}
...
sub load_profile($) {
	...
	webmin_log("load", "profile", $profile_name, undef);
