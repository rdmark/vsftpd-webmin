...
sub read_config($) {
	...
	$self->{LINES} = main::read_file_lines($self->{CONFIG_FILE});
	return 1;
}
...
sub persist() {
	...
	main::lock_file($self->{CONFIG_FILE});
	main::flush_file_lines($self->{CONFIG_FILE});
	main::unlock_file($self->{CONFIG_FILE});	
	umask($mask);
	
	main::webmin_log("persist", "lineconfig", $self->{NAME}, undef);
